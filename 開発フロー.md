...
## 単体テストの自動実行
102. ターミナルでカレントディレクトリを各関数フォルダに移す
     ```bash
     cd function-[機能名]
     ```
     または
     ...
     ```
105. testsディレクトリを新規作成して移動する
     ```bash
     mkdir ./tests
     cd tests
     ```
106. testsディレクトリ内に`__init__.py`と`func_test.py`を新規作成し、`func_test.py`をアクティブにする
     ```bash
...
201. yamlファイル
202. 23行目にある`branches`の`branch_name`をpush先のブランチ名（`feature/[作業名]`）に変更する
203. 26行目にある`AZURE_FUNCTIONAPP_NAME`および27行目にある`AZURE_FUNCTIONAPP_PACKAGE_PATH`の`your_app_name`を関数フォルダ名（`function-[機能名]`）に変更する
204. 79行目にある`publish-profile`の`your_profile`を`AZURE_FUNCTIONAPP_PUBLISH_PROFILE_XXX`に変更する（XXXの部分は機能に応じて適当に命名する）
