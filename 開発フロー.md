...
## ローカル環境でのコード生成とデバッグ
==========初回のみ実施==========
- back/local.settings.jsonのValuesにCONNECTION_STRINGを追加
...
9. ターミナルでF5キーを押してデバッグ
   - http://localhost:7071/api/[機能名] を開く
   - GETリクエストの結果を見たい場合は、URLの末尾に`?[パラメータ名1]=[値1]&[パラメータ名2]=[値2]&...`を追加する
10. 結果を確認し、`ctrl`+`C`で実行を停止する
11. 期待する結果が表示されたら次へ、そうでなければ[8.](#back_coding)に戻る
